...
public class ViewBanner extends FrameLayout implements OnPageChangeListener {
    public String tag = "bannerview";
    private int mIndicatorMargin = BannerConfig.PADDING_SIZE;
    private int mIndicatorWidth;
    private int mIndicatorHeight;
    private int indicatorSize;
    private int defaultRedsId = R.layout.layout_banner_nodata;
    ...
    private LinearLayout indicator, indicatorInside, titleView, default_layout;
    ...
    public ViewBanner(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        default_layout = (LinearLayout) view.findViewById(R.id.default_layout);
    ...
    public void setDefaultLayout(int RedsId) {
        defaultRedsId = RedsId;
        View defaultView = LayoutInflater.from(context).inflate(defaultRedsId, this, false);
        default_layout.removeAllViews();
        default_layout.addView(defaultView);
    }
    ...
    public ViewBanner isAutoPlay(boolean isAutoPlay) {
        this.isAutoPlay = isAutoPlay;
        return this;
    }

    public ViewBanner setViewLoader(ImageLoaderInterface imageLoader) {
        this.imageLoader = imageLoader;
        return this;
    }

    public ViewBanner setDelayTime(int delayTime) {
        this.delayTime = delayTime;
        return this;
    }

    public ViewBanner setIndicatorGravity(int type) {
        switch (type) {
    ...
    public ViewBanner setBannerAnimation(Class<? extends PageTransformer> transformer) {
        try {
     ...
     * @return Banner
     */
    public ViewBanner setOffscreenPageLimit(int limit) {
        if (viewPager != null) {
     ...
     * @return Banner
     */
    public ViewBanner setPageTransformer(boolean reverseDrawingOrder, PageTransformer transformer) {
        viewPager.setPageTransformer(reverseDrawingOrder, transformer);
        return this;
    }

    public ViewBanner setBannerTitles(List<String> titles) {
        this.titles = titles;
        return this;
    }

    public ViewBanner setBannerStyle(int bannerStyle) {
        this.bannerStyle = bannerStyle;
        return this;
    }

    public ViewBanner setViewPagerIsScroll(boolean isScroll) {
        this.isScroll = isScroll;
        return this;
    }

    public ViewBanner setImages(List<?> imageUrls) {
    ...
    private void setImageList(List<?> imagesUrl) {
        if (imagesUrl == null || imagesUrl.size() <= 0) {
            default_layout.setVisibility(VISIBLE);
            Log.e(tag, "The image data set is empty.");
            return;
        }
        default_layout.setVisibility(GONE);
        initImages();
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (view == null) {
                view = LayoutInflater.from(context).inflate(R.layout.recycler_white, null);
            }
            Object data = null;
            if (i == 0) {
                data = imagesUrl.get(count - 1);
            } else if (i == count + 1) {
                data = imagesUrl.get(0);
            } else {
                data = imagesUrl.get(i - 1);
            }
            list.add(view);
            if (imageLoader != null)
                imageLoader.displayData(context, data, view);
            else
     ...
     * @return
     */
    public ViewBanner setOnBannerListener(OnBannerListener listener) {
